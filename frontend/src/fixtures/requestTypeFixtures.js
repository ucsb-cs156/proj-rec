const restaurantFixtures = {
    oneRestaurant: [
      {
        id: 1,
        name: "CS Department BS/MS program",
        description: "Request for recommendation for CS Department BS/MS program",
      },
    ],
  
    threeRestaurants: [
      {
        id: 2,
        name: "Scholarship or Fellowship",
        description:
          "Request for recommendation for a scholarship or fellowship",
      },
  
      {
        id: 3,
        name: "MS program (other than CS Dept BS/MS)",
        description: "Request for recommendation for MS program",
      },
  
      {
        id: 4,
        name: "PhD program",
        description:
          "Request for recommendation for PhD program",
      },
      {
        id: 5,
        name: "Other",
        description:
          "Any other request type not listed"
      },
    ],
  };
  
  export { requestTypeFixtures };
  